...
class ChatEngine:
    ...
    def generate_stream(self, user_input: str) -> Iterator[str]:
        ...
        try:
            ...
            # B23 修复: 空回复不污染对话历史，回滚 user 消息
            if full_response.strip():
                self.add_to_history("assistant", full_response)
            else:
                error_message = "\n[WARN] Empty response, rolling back user message"
                print(error_message)
                yield error_message
                if (self.sessions[self.active_session]
                        and self.sessions[self.active_session][-1]["role"] == "user"):
