...
# 公共 flags
# -fit off: 禁用自动内存适配（板卡 UMA 内存检测有 underflow bug）
# --no-warmup 暴力跳过 warm-up（warmup 在 SYCL 上慢，且会撞 240s timeout）
COMMON="-ngl 99 -ngld 0 -c 2048 -ctk q8_0 -ctv f16 --temp 0 -fit off"
TIMEOUT_S=420  # 7 分钟，给 35B 模型加载留余量
...
run_one() {
    ...
    timeout $TIMEOUT_S "$@" < /dev/null > "$OUT/${name}.log" 2>&1
    local ec=$?
    set -e
    # 抓关键性能行打印到 main
